...
export const metadata: Metadata = {
  title: "Hair services",
  description: `Cuts, color, treatments, styling, and bridal hair at ${site.name} — full menu in Lahore.`,
};
...
export default function HairServicesPage() {
  return (
    <div className="pt-28">
        ...
        <div className="mx-auto max-w-7xl">
          <Reveal>
            ...
            <p className="mt-6 max-w-2xl text-white/60">
              Explore our full hair offering — from precision cuts and fashion
              color to treatments, styling, and bridal artistry. Tap a card to
              book that service.
            </p>
      ...
      {hairServiceSections.map((section, si) => (
        <section
          key={section.id}
          id={section.id}
                ...
                <Reveal key={item.name} delay={Math.min(idx * 0.03, 0.24)}>
                  <article className="group flex h-full flex-col rounded-2xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-sm transition hover:border-gold/30 hover:bg-white/[0.06]">
                    ...
                    <Link
                      ...
                      className="mt-5 inline-flex w-fit rounded-full border border-gold/40 bg-gold/5 px-5 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-gold transition hover:border-gold hover:bg-gold/15"
                    >
                      Book now
                    </Link>
                  </article>
        ...
        <div className="mx-auto max-w-3xl text-center">
          <Reveal>
            <p className="text-sm text-white/55">
              Not sure which service fits? Book a consultation and we will map a
              plan for your hair goals.
            </p>
            <Link
